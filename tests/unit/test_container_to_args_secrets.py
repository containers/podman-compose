...
class TestContainerToArgsSecrets(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_secret_string_options_external_name_in_declared_secrets(self):
        ...
        self.assertEqual('That is un-supported and a no-op and is ignored.' in cm.output[0], True)
        self.assertEqual('my_secret_name' in cm.output[0], True)
    ...
    @parameterized.expand([
        (
            ...
            "file_secret",
            "/test_dirname/my_secret:/run/secrets/file_secret:ro,rprivate,rbind",
        ),
        (
            "custom_target_name",
            {
            ...
            "/test_dirname/my_secret:/run/secrets/custom_name:ro,rprivate,rbind",
        ),
        (
        ...
        (
            ...
            "/test_dirname/my_secret:/etc/custom_location:ro,rprivate,rbind",
        ),
    ])
    async def test_file_secret(
        self, test_name, declared_secrets, add_to_minimal_container, volume_ref
    ):
        c = create_compose_mock()
        c.declared_secrets = declared_secrets
        cnt = get_minimal_container()
    ...
    async def test_file_secret_unused_params_warning(self):
        ...
        with self.assertLogs() as cm:
            args = await container_to_args(c, cnt)
        self.assertEqual(len(cm.output), 1)
        self.assertEqual('with uid, gid, or mode.' in cm.output[0], True)
        self.assertEqual('unused_params_warning' in cm.output[0], True)
