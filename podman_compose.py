...
def dicts_get(dicts, key, fallback="", fallback_empty=False):
    ...
    If not found in any, then use fallback. If fallback is Exception raise it.
    """
    value = None
    for d in dicts:
        value = d.get(key)
...
def rec_subs(value, dicts):
    """
    Do bash-like substitution in value and if list of dictionary do that
    recursively.
    """
    if is_dict(value):
...
def norm_as_list(src):
    ...
    return a list of ["key1=value1", "key2"] values.
    """
    if src is None:
        dst = []
    elif is_dict(src):
...
def norm_as_dict(src):
    ...
    return a dictionary {key1:value1, key2: None} values.
    """
    if src is None:
        dst = {}
    elif is_dict(src):
...
def move_list(dst, containers, key):
    """
    Move key (like port forwarding) from containers to dst (a pod or a infra
    container).
    """
...
def tr_cntnet(project_name, services, given_containers):
    ...
    infra = dict(name=infra_name, image="k8s.gcr.io/pause:3.1")
    for cnt0 in given_containers:
...
def mount_dict_vol_to_bind(compose, mount_dict):
    """
    Inspect volume to get directory. Create volume if needed and return
    mount_dict as bind of that directory.
    """
    ...
    # podman volume list --format '{{.Name}}\t{{.MountPoint}}' -f \
    # 'label=io.podman.compose.project=HERE'
    try:
...
def mount_desc_to_args(compose, mount_desc, srv_name, cnt_name):
    ...
    mount_desc = mount_dict_vol_to_bind(
        compose, fix_mount_dict(mount_desc, proj_name, srv_name)
    )
    ...
    if mount_desc.get("bind"):
        ...
        if bind_prop:
            opts.append("bind-propagation={}".format(bind_prop))
    ...
    if mount_type == "bind":
        return "type=bind,source={source},destination={target},{opts}".format(
            source=source, target=target, opts=opts
        ).rstrip(",")
...
def container_to_args(compose, cnt, detached=True, podman_command="run"):
    ...
    shared_vols = compose.shared_vols
    pod = cnt.get("pod") or ""
    podman_args = [podman_command, "--name={}".format(cnt.get("name"))]
    ...
    if healthcheck_test:
        ...
        if is_str(healthcheck_test):
            ...
            podman_args.extend(
                [
                    "--healthcheck-command",
                    "/bin/sh -c {}".format(cmd_quote(healthcheck_test)),
                ]
            )
            ...
            if healthcheck_type == "NONE":
                ...
                podman_args.extend(
                    [
                        "--healthcheck-command",
                        "/bin/sh -c {}".format(cmd_quote(healthcheck_test[0])),
                    ]
                )
            else:
                raise ValueError(
...
def build_one(compose, args, cnt):
    ...
    dockerfile = os.path.join(
        ctx,
        build_desc.get("dockerfile", "Dockerfile")
    )
    ...
    build_args = ["build", "-t", cnt["image"], "-f", dockerfile]
    if getattr(args, "pull_always"):
        build_args.append("--pull-always")
    elif getattr(args, "pull"):
...
def compose_up_parse(parser):
    parser.add_argument(
        ...
        "--detach",
        action="store_true",
        help="Detached mode: Run container in the background, print new "
             "container name. Incompatible with --abort-on-container-exit.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--force-recreate",
        action="store_true",
        help="Recreate containers even if their configuration and image "
             "haven't changed.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--no-recreate",
        action="store_true",
        help="If containers already exist, don't recreate them. Incompatible "
             "with --force-recreate and -V.",
    )
    parser.add_argument(
        "--no-build",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=float,
        default=10,
        help="Use this timeout in seconds for container shutdown when "
             "attached or when containers are already running. (default: 10)",
    )
    parser.add_argument(
        "-V",
        "--renew-anon-volumes",
        action="store_true",
        help="Recreate anonymous volumes instead of retrieving data from "
    ...
    parser.add_argument(
        ...
        metavar="SERVICE=NUM",
        action="append",
        help="Scale SERVICE to NUM instances. Overrides the `scale` setting "
             "in the Compose file if present.",
    )
    parser.add_argument(
        "--exit-code-from",
        metavar="SERVICE",
...
def compose_run_parse(parser):
    ...
    parser.add_argument(
        ...
        "--publish",
        action="append",
        help="Publish a container's port(s) to the host (can be used "
             "multiple times)",
    )
    parser.add_argument(
        "--service-ports",
        action="store_true",
        help="Run command with the service's ports enabled and mapped to the "
             "host.",
    )
    parser.add_argument(
        "-v",
    ...
    parser.add_argument(
        "-T",
        action="store_true",
        help="Disable pseudo-tty allocation. By default `podman-compose run` "
             "allocates a TTY.",
    )
    parser.add_argument(
        "-w",
...
def compose_parse_services(parser):
    parser.add_argument(
        "services", metavar="services", nargs="+", help="affected services"
    )
...
def compose_push_parse(parser):
    parser.add_argument(
        ...
        action="store_true",
        help="Push what it can and ignores images with push failures. (not "
             "implemented)",
    )
    parser.add_argument(
        "services",
...
def compose_build_parse(parser):
    ...
    parser.add_argument(
        "--pull-always",
        action="store_true",
        help="attempt to pull a newer version of the image, Raise an error "
